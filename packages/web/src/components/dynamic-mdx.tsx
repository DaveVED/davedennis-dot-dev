...
const components: Record<string, React.FC<MDXComponentProps>> = {
  h1: ({ className, ...props }) => (
    <h1
      className={cn(
...
const DynamicMdx: React.FC<DynamicMdxProps> = ({ children }) => {
  const [Content, setContent] = useState<React.ComponentType<{ components: typeof components }> | null>(null);
    ...
    const compileMdx = async () => {
      try {
        const { default: ContentComponent } = await evaluate(children, {
          ...runtime,
          development: false,
          rehypePlugins: [[rehypePrettyCode, { theme: 'github-dark' }]], // Set your theme here
        });
